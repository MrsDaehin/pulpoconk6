...
export function submitCheckout() {
  ...
  group("Submit Checkout", function () {
    ...
    try {
      result = jsonpath.query(response.json(), "$['result']")[0];
    } catch (err) {
    ...
    globalThis.vars["orderId"] = findBetween(
      globalThis.vars["redirectUrl"],
      "order-received/",
      "/"
    );
    globalThis.vars["key"] = globalThis.vars["redirectUrl"].substring(
      globalThis.vars["redirectUrl"].indexOf("key=") + 4
    );
    ...
    if (globalThis.vars["orderId"].length > 0) {
      console.log(
        "Successfully placed order! ID: " + globalThis.vars["orderId"]
      );
    } else {
      if (response.body) {
    ...
    response = http.get(globalThis.vars["redirectUrl"], {
      tags: {
    ...
    checkStatus({
      ...
      failOnError: true,
      printOnError: true,
      dynamicIds: [globalThis.vars["orderId"], globalThis.vars["key"]],
    });
